...
if (is_post()) {
  ...
  try {
    if ($action === 'create') {
      ...
      $id = (int)$_POST['id'];
      $current = $pdo->prepare('SELECT photo FROM users WHERE id = ? AND role="student"');
      $current->execute([$id]);
      $old = $current->fetch();
...
<div class="dashboard-shell"><?php include dirname(dirname(__DIR__)) . '/includes/sidebar.php'; ?><main class="main-panel">
    <div class="dashboard-top">
      <div class="dashboard-title">
        ...
        <p>Keep student details complete with course, class, contact, guardian details, and profile photo.</p>
      </div>
      <div class="user-chip">Student Records</div>
    </div>
      ...
      <div class="form-card">
        ...
        <form method="post" enctype="multipart/form-data">
          ...
          <div class="form-grid">
            ...
            <div class="form-group"><label>Class <span class="required">*</span></label><select name="class_filter">
                <option value="">Select Class</option><?php foreach ($classes as $class): ?><option value="<?= e($class) ?>"><?= e($class) ?></option><?php endforeach; ?>
              </select></div>
